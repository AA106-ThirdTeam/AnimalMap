...
<html>
<head>
<title>自家寵物留言新增 - addPet_Message.jsp</title>
...
<table border='1' cellpadding='5' cellspacing='0' width='800'>
	<tr bgcolor='#CCCCFF' align='center' valign='middle' height='20'>
		...
		<td>
		   <a href="<%=request.getContextPath()%>/Heibernate_back-end/pet_message/select_page.jsp"><img src="<%=request.getContextPath()%>/images/tomcat.gif" width="100" height="100" border="1">回首頁</a>
	    </td>
	</tr>
</table>
<h3>自家寵物留言:</h3>
	...
	<tr>
		...
		<td><select size="1" name="pet_Id">
			<c:forEach var="petVO" items="${petSvc.all}">
				<option value="${petVO.pet_Id}" ${(pet_messageVO.petVO.pet_Id==petVO.pet_Id)? 'selected':'' } >${petVO.pet_Id}
			</c:forEach>
		</select></td>
	</tr>
	...
	<tr>
		...
		<td><select size="1" name="mem_Id">
			<c:forEach var="memVO" items="${memSvc.all}">
				<option value="${memVO.mem_Id}" ${(pet_messageVO.memVO.mem_Id==memVO.mem_Id)? 'selected':'' } >${memVO.mem_Id}
			</c:forEach>
		</select></td>
	</tr>
	<tr>
