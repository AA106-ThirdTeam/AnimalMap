 ...
 *	英文:shopPhoto<br>
 */ 
@WebServlet(urlPatterns = { "/back-end/shopphoto/shopphoto.do" })
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("getOne_For_Display".equals(action)) { 
			...
			try {
				...
				String url = "/back-end/shopphoto/listOneShopPhoto.jsp";
		...
		if ("getOne_For_Update".equals(action)) {
			...
			try {			
				...
				/***************************3.查詢完成,準備轉交(Send the Success view)*************/
				req.setAttribute("shopphotoVO", shopphotoVO); 		
				String url = "/back-end/shopphoto/update_shopphoto_input.jsp";
		...
		if ("update".equals(action)) { // 來自update_shopPhoto_input.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back-end/shopphoto/update_shopphoto_input.jsp");
					failureView.forward(req, res);
					return; //程式中斷
				}
				...
				if(requestURL.equals("/back-end/shopphoto/listShopPhotos_ByShop_Id.jsp") 
					|| requestURL.equals("/back-end/shopphoto/listAllShopPhoto.jsp")){
					req.setAttribute("listShopPhotos_ByShop_Id",shopphotoSvc.getShopPhotosByShop_Id(shopPhoto_ShopId)); // 資料庫取出的list物件,存入request
				}
				
				
				if(requestURL.equals("/back-end/shopphoto/listShopPhotos_ByCompositeQuery.jsp")){
        ...
        if ("insert".equals(action)) { // 來自addShopPhoto.jsp的請求  
			...
			try {
                ...
                if (!errorMsgs.isEmpty()) {
                    req.setAttribute("shopphotoVO", shopphotoVO); // 含有輸入格式錯誤的shopphotoVO物件,也存入req
                    RequestDispatcher failureView = req.getRequestDispatcher("/back-end/shopphoto/addShopPhoto.jsp");
                    failureView.forward(req, res);
                    return;
                }
				...
				String url = "/back-end/shopphoto/listAllShopPhoto.jsp";
			...
			/***************************其他可能的錯誤處理**********************************/
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/back-end/shopphoto/addShopPhoto.jsp");
				failureView.forward(req, res);
			}
		}			
		...
		if ("delete".equals(action)) { 
			...
			try {
				...
				if(requestURL.equals("/back-end/shopphoto/listShopPhotos_ByShop_Id.jsp") 
					|| requestURL.equals("/back-end/shopphoto/listAllShopPhoto.jsp")){
					req.setAttribute("listShopPhotos_ByShop_Id",shopphotoSvc.getShopPhotosByShop_Id(shopphotoVO.getShopPhoto_ShopId())); // 資料庫取出的list物件,存入request
				}
        ...
        if ("listShopPhotos_ByCompositeQuery".equals(action)) { // 來自select_page.jsp的複合查詢請求
            List<String> errorMsgs = new LinkedList<String>();
            // Store this set in the request scope, in case we need to
            // send the ErrorPage view.
            req.setAttribute("errorMsgs", errorMsgs);
            ...
            try {
                ...
                RequestDispatcher successView = req.getRequestDispatcher("/back-end/shopphoto/listShopPhotos_ByCompositeQuery.jsp"); // 成功轉交listShopPhotos_ByCompositeQuery.jsp
                successView.forward(req, res);
        ...
        // 來自select_page.jsp的請求                                  // 來自 shopphoto/listAllShopPhoto.jsp的請求
        if ("listShopPhotos_ByShop_Id_A".equals(action)) {

            List<String> errorMsgs = new LinkedList<String>();
            req.setAttribute("errorMsgs", errorMsgs);
            try {
                /*************************** 1.接收請求參數 ****************************************/
                String shopPhoto_ShopId = new String(req.getParameter("shopPhoto_ShopId"));

                /*************************** 2.開始查詢資料 ****************************************/
                ShopPhotoService shopphotoSvc = new ShopPhotoService();
                Set<ShopPhotoVO> set = shopphotoSvc.getShopPhotosByShop_Id(shopPhoto_ShopId);

                /*************************** 3.查詢完成,準備轉交(Send the Success view) ************/
                req.setAttribute("listShopPhotos_ByShop_Id", set);    // 資料庫取出的list物件,存入request

                String url = null;
                if ("listShopPhotos_ByShop_Id_A".equals(action))
                    url = "/back-end/shopphoto/listShopPhotos_ByShop_Id.jsp";        // 成功轉交 shopphoto/listShopPhotos_ByShop_Id.jsp
