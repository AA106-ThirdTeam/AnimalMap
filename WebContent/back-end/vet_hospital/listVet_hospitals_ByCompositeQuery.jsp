...
<jsp:useBean id="listVet_hospitals_ByCompositeQuery" scope="request" type="java.util.List" />
...
<html>
<head>
<title>複合查詢 - listVet_hospitals_ByCompositeQuery.jsp</title>
	<!-- 共用HEAD -->
...
<table border='1' cellpadding='5' cellspacing='0' width='800'>
    <tr bgcolor='#CCCCFF' align='center' valign='middle' height='20'>
        <td>
        <h3><font color=red>複合查詢</font>診所 - listVet_hospitals_ByCompositeQuery.jsp</h3>
        <a href="<%=request.getContextPath()%>/back-end/vet_hospital/select_page.jsp"><img src="<%=request.getContextPath()%>/back-end/images/back1.gif" width="100" height="32" border="0">回首頁</a>
        </td>
    </tr>
</table>
...
<%  
    rowNumber=listVet_hospitals_ByCompositeQuery.size();
...
<!-- page1_ByCompositeQuery end -->
    ...
    <c:forEach var="vet_hospitalVO" items="${listVet_hospitals_ByCompositeQuery}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
        <tr align='center' valign='middle' ${(vet_hospitalVO.hos_Id==param.hos_Id) ? 'bgcolor=#CCCCFF':''}><!--將修改的那一筆加入對比色而已-->    
            ...
            <td>
                <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/back-end/vet_hospital/vet_hospital.do">
 ...
 <tr>
  <%if (rowsPerPage<rowNumber) {%>
    <%if(pageIndex>=rowsPerPage){%>
        <td><A href="<%=request.getContextPath()%>/vet_hospital/vet_hospital.do?whichPage=1&action=listVet_hospitals_ByCompositeQuery">至第一頁</A>&nbsp;</td>
        <td><A href="<%=request.getContextPath()%>/vet_hospital/vet_hospital.do?whichPage=<%=whichPage-1%>&action=listVet_hospitals_ByCompositeQuery">上一頁 </A>&nbsp;</td>
    <%}%>
  
    <%if(pageIndex<pageIndexArray[pageNumber-1]){%>
        <td><A href="<%=request.getContextPath()%>/vet_hospital/vet_hospital.do?whichPage=<%=whichPage+1%>&action=listVet_hospitals_ByCompositeQuery">下一頁 </A>&nbsp;</td>
        <td><A href="<%=request.getContextPath()%>/vet_hospital/vet_hospital.do?whichPage=<%=pageNumber%>&action=listVet_hospitals_ByCompositeQuery">至最後一頁</A>&nbsp;</td>
    <%}%>
  <%}%>  
 </tr>
</table>    
 ...
 <tr> 
   ...
   <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/vet_hospital/vet_hospital.do">   
        ...
        <%}%> 
           ...
           <input type="hidden" name="action"  value="listVet_hospitals_ByCompositeQuery">  
        </td>
   </FORM>
 </tr>
</table>
