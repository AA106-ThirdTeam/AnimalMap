 ...
 *	英文:pet_Photos<br>
 */ 
@WebServlet(urlPatterns = { "/back-end/pet_photos/pet_photos.do" })
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("getOne_For_Display".equals(action)) { 
			...
			try {
				...
				String url = "/back-end/pet_photos/listOnePet_Photos.jsp";
		...
		if ("getOne_For_Update".equals(action)) {
			...
			try {			
				...
				/***************************3.查詢完成,準備轉交(Send the Success view)*************/
				req.setAttribute("pet_photosVO", pet_photosVO); 		
				String url = "/back-end/pet_photos/update_pet_photos_input.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url); // 成功轉交update_pet_photos_input.jsp
				successView.forward(req, res);
		...
		if ("update".equals(action)) { // 來自update_pet_Photos_input.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back-end/pet_photos/update_pet_photos_input.jsp");
					failureView.forward(req, res);
					return; //程式中斷
				}
				...
				if(requestURL.equals("/back-end/pet_photos/listPet_Photoss_ByPet_Id.jsp") 
					|| requestURL.equals("/back-end/pet_photos/listAllPet_Photos.jsp")){
					req.setAttribute("listPet_Photoss_ByPet_Id",pet_photosSvc.getPet_PhotossByPet_Id(pet_Id)); // 資料庫取出的list物件,存入request
				}

				if(requestURL.equals("/back-end/pet_photos/listPet_Photoss_ByMem_Id.jsp") 
					|| requestURL.equals("/back-end/pet_photos/listAllPet_Photos.jsp")){
					req.setAttribute("listPet_Photoss_ByMem_Id",pet_photosSvc.getPet_PhotossByMem_Id(mem_Id)); // 資料庫取出的list物件,存入request
				}
				
				
				if(requestURL.equals("/back-end/pet_photos/listPet_Photoss_ByCompositeQuery.jsp")){
        ...
        if ("insert".equals(action)) { // 來自addPet_Photos.jsp的請求  
			...
			try {
                ...
                if (!errorMsgs.isEmpty()) {
                    req.setAttribute("pet_photosVO", pet_photosVO); // 含有輸入格式錯誤的pet_photosVO物件,也存入req
                    RequestDispatcher failureView = req.getRequestDispatcher("/back-end/pet_photos/addPet_Photos.jsp");
                    failureView.forward(req, res);
                    return;
                }
				...
				String url = "/back-end/pet_photos/listAllPet_Photos.jsp";
			...
			/***************************其他可能的錯誤處理**********************************/
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/back-end/pet_photos/addPet_Photos.jsp");
				failureView.forward(req, res);
			}
		}			
		...
		if ("delete".equals(action)) { 
			...
			try {
				...
				if(requestURL.equals("/back-end/pet_photos/listPet_Photoss_ByPet_Id.jsp") 
					|| requestURL.equals("/back-end/pet_photos/listAllPet_Photos.jsp")){
					req.setAttribute("listPet_Photoss_ByPet_Id",pet_photosSvc.getPet_PhotossByPet_Id(pet_photosVO.getPet_Id())); // 資料庫取出的list物件,存入request
				}

				if(requestURL.equals("/back-end/pet_photos/listPet_Photoss_ByMem_Id.jsp") 
					|| requestURL.equals("/back-end/pet_photos/listAllPet_Photos.jsp")){
					req.setAttribute("listPet_Photoss_ByMem_Id",pet_photosSvc.getPet_PhotossByMem_Id(pet_photosVO.getMem_Id())); // 資料庫取出的list物件,存入request
				}
        ...
        if ("listPet_Photoss_ByCompositeQuery".equals(action)) { // 來自select_page.jsp的複合查詢請求
            List<String> errorMsgs = new LinkedList<String>();
            // Store this set in the request scope, in case we need to
            // send the ErrorPage view.
            req.setAttribute("errorMsgs", errorMsgs);
            ...
            try {
                ...
                RequestDispatcher successView = req.getRequestDispatcher("/back-end/pet_photos/listPet_Photoss_ByCompositeQuery.jsp"); // 成功轉交listPet_Photoss_ByCompositeQuery.jsp
                successView.forward(req, res);
        ...
        // 來自select_page.jsp的請求                                  // 來自 pet_photos/listAllPet_Photos.jsp的請求
        if ("listPet_Photoss_ByPet_Id_A".equals(action)) {
            ...
            try {
                ...
                /*************************** 2.開始查詢資料 ****************************************/
                Pet_PhotosService pet_photosSvc = new Pet_PhotosService();
                Set<Pet_PhotosVO> set = pet_photosSvc.getPet_PhotossByPet_Id(pet_Id);

                /*************************** 3.查詢完成,準備轉交(Send the Success view) ************/
                req.setAttribute("listPet_Photoss_ByPet_Id", set);    // 資料庫取出的list物件,存入request

                String url = null;
                if ("listPet_Photoss_ByPet_Id_A".equals(action))
                    url = "/back-end/pet_photos/listPet_Photoss_ByPet_Id.jsp";        // 成功轉交 pet_photos/listPet_Photoss_ByPet_Id.jsp
        ...
        // 來自select_page.jsp的請求                                  // 來自 pet_photos/listAllPet_Photos.jsp的請求
        if ("listPet_Photoss_ByMem_Id_A".equals(action)) {
            ...
            try {
                ...
                /*************************** 2.開始查詢資料 ****************************************/
                Pet_PhotosService pet_photosSvc = new Pet_PhotosService();
                Set<Pet_PhotosVO> set = pet_photosSvc.getPet_PhotossByMem_Id(mem_Id);

                /*************************** 3.查詢完成,準備轉交(Send the Success view) ************/
                req.setAttribute("listPet_Photoss_ByMem_Id", set);    // 資料庫取出的list物件,存入request

                String url = null;
                if ("listPet_Photoss_ByMem_Id_A".equals(action))
                    url = "/back-end/pet_photos/listPet_Photoss_ByMem_Id.jsp";        // 成功轉交 pet_photos/listPet_Photoss_ByMem_Id.jsp
