...
<html>
<head>
<title>複合查詢 - listProducts_ByCompositeQuery.jsp</title>
	<!-- 共用HEAD -->
...
<table border='1' cellpadding='5' cellspacing='0' width='800'>
    <tr bgcolor='#CCCCFF' align='center' valign='middle' height='20'>
        <td>
        <h3><font color=red>複合查詢</font>商品 - listProducts_ByCompositeQuery.jsp</h3>
        <a href="<%=request.getContextPath()%>/back-end/product/select_page.jsp"><img src="<%=request.getContextPath()%>/back-end/images/back1.gif" width="100" height="32" border="0">回首頁</a>
        </td>
    </tr>
</table>
...
<table border='1' bordercolor='#CCCCFF'>
    <tr>
		<th>商品編號</th>		<th>商品名稱</th>		<th>商品簡介</th>		<th>商品價格</th>		<th>商品庫存量</th>		<th>商品圖片</th>		<th>商品圖片（縮圖）</th>		<th>商品上下架狀態</th>		<th>商品建立日期</th>		<th>商品資訊</th>		<th>商品類別編號</th>  
        <th>修改</th>
        <th>刪除</th>
    </tr>
...
<%  
    rowNumber=listProducts_ByCompositeQuery.size();
 ...
 <tr>
  <%if (rowsPerPage<rowNumber) {%>
    <%if(pageIndex>=rowsPerPage){%>
        <td><A href="<%=request.getContextPath()%>/product/product.do?whichPage=1&action=listProducts_ByCompositeQuery">至第一頁</A>&nbsp;</td>
        <td><A href="<%=request.getContextPath()%>/product/product.do?whichPage=<%=whichPage-1%>&action=listProducts_ByCompositeQuery">上一頁 </A>&nbsp;</td>
    <%}%>
  
    <%if(pageIndex<pageIndexArray[pageNumber-1]){%>
        <td><A href="<%=request.getContextPath()%>/product/product.do?whichPage=<%=whichPage+1%>&action=listProducts_ByCompositeQuery">下一頁 </A>&nbsp;</td>
        <td><A href="<%=request.getContextPath()%>/product/product.do?whichPage=<%=pageNumber%>&action=listProducts_ByCompositeQuery">至最後一頁</A>&nbsp;</td>
    <%}%>
  <%}%>  
 </tr>
</table>    
