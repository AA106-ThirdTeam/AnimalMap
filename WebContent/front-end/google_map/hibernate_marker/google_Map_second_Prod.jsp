...
<%
    ...
    List<Second_ProdVO> list_second_prod =(List)session.getAttribute("list_second_Prod");
    //pageContext.setAttribute("list_second_prod",list_second_prod);
    int second_prod_map_icon_size = 48;
%>
<style>
...
%>
<div id=ex_animal_map_second_Prod_<%=vo.getSecond_Prod_Id()%> value="<%=vo.getSecond_Prod_Id()%>" hidden>
        <div style="width: 20vw;">
            <div class="">
                ...
                <h4>
                	<%=vo.getSecond_Prod_Title()%>
                </h4>
                <hr>
                <p>
        			<%=vo.getSecond_Prod_Content()%>
        		</p>
            </div>
        </div> 		
        <hr>
        <button onclick="show_second_Prod_details_page('<%=vo.getSecond_Prod_Id()%>')"
...
// 					    // 標記 ID
// 					    // Custom ID
					    id: 'marker_second_prod_<%=vo.getSecond_Prod_Id()%>'
					    ,index:'<%=vo.getSecond_Prod_Id()%>'
					    ,type:'second_prod'
// 					    // 標記的位置
// 					    // Marker location
					    ,addr: ['<%=vo.getSecond_Prod_Lon()%>', '<%=vo.getSecond_Prod_Lat()%>']
...
// 					    // Content of infoWindow
					    ,text: 
					    	'<div id ="div_second_Prod_<%=tem_int%>">'
					    	+'<button onclick="show_second_Prod_details_page(this.value)"'
					    	+ 'class="btn .btn-md btn-block btn-info" >詳細資料!</button>'
					    	+ '</div>'
				    	,text_html:"ex_animal_map_second_Prod_<%=vo.getSecond_Prod_Id()%>"
// 					    // 標籤文字層，顯示於標記底下
...
// 					    // 或是 Object 定義更詳細的圖示
			            ,'icon': {
			            	 // 圖示網址
			                'url': 'https://i.imgur.com/rYf3UTV.png'
		                	// 縮放尺寸
			                ,'scaledSize': [<%=second_prod_map_icon_size%>, <%=second_prod_map_icon_size%>]
			            }
...
// 					    },
					    ...
					    ,event       : {
                            ...
                            mouseout: {
                                func: function () {
                                    //if(infowindow_second_Prod_<%=tem_int%>!=null){
                                        //infowindow_second_Prod_<%=tem_int%>.close(map,this);
                                    //}
                                }
                            }
					    }
...
<script type="text/javascript">
	function show_second_Prod_details_page(pk_value) {
		var path_parameter = 'action=getOne_For_Display&Id=' + pk_value;
		var src='<%=request.getContextPath()%>/Heibernate_front-end/marker_detail_infowindow/second_Prod_detail_page.jsp?'+path_parameter 
