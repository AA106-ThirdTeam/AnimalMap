...
import jdbc.util.CompositeQuery.jdbcUtil_CompositeQuery_Stray_Ani_photos;
/** 
 *表格名稱 : <br>
 *	社區流浪動物相簿<br>
...
public class Stray_Ani_photosDAO implements Stray_Ani_photosDAO_interface{
	...
	//====以下是一般指令====
	public static final String INSERT_STMT = "INSERT INTO stray_Ani_photos(str_Ani_Pic_No,stray_Ani_Id,mem_Id,stray_Ani_Pic,stray_Pic_name,stray_Pic_extent,stray_Pic_time,stray_Pic_type ) VALUES  ( stray_Ani_photos_seq1.nextval , ? , ? , ? , ? , ? , ? , ?  )  " ; 
	//====以下是更新指令====
	...
	//====以下是單筆資料查詢指令====
	public static final String GET_ONE_STMT = "SELECT str_Ani_Pic_No,stray_Ani_Id,mem_Id,stray_Ani_Pic,stray_Pic_name,stray_Pic_extent,to_char(stray_Pic_time,'yyyy-mm-dd') stray_Pic_time,stray_Pic_type FROM stray_ani_photos WHERE str_Ani_Pic_No=? " ; 
	//====以下是單筆資料查詢指令====
	public static final String GET_ALL_STMT = "SELECT str_Ani_Pic_No,stray_Ani_Id,mem_Id,stray_Ani_Pic,stray_Pic_name,stray_Pic_extent,to_char(stray_Pic_time,'yyyy-mm-dd') stray_Pic_time,stray_Pic_type FROM stray_Ani_photos order by str_Ani_Pic_No " ; 

	//====以下是GET_ByFK_STMT指令====
    private static final String GET_Stray_Ani_photoss_ByStray_Ani_Id_STMT = 
		"SELECT str_Ani_Pic_No,stray_Ani_Id,mem_Id,stray_Ani_Pic,stray_Pic_name,stray_Pic_extent,to_char(stray_Pic_time,'yyyy-mm-dd') stray_Pic_time,stray_Pic_type FROM Stray_Ani_photos WHERE stray_Ani_Id = ? order by stray_Ani_Id";
    private static final String GET_Stray_Ani_photoss_ByMem_Id_STMT = 
		"SELECT str_Ani_Pic_No,stray_Ani_Id,mem_Id,stray_Ani_Pic,stray_Pic_name,stray_Pic_extent,to_char(stray_Pic_time,'yyyy-mm-dd') stray_Pic_time,stray_Pic_type FROM Stray_Ani_photos WHERE mem_Id = ? order by mem_Id";
	...
	public void update(Stray_Ani_photosVO aStray_Ani_photosVO){
		...
		try {
			...
			pstmt.setString (6, aStray_Ani_photosVO.getStr_Ani_Pic_No());
			pstmt.executeUpdate();
		} catch (SQLException se) {
			throw new RuntimeException("A database error occured. " + se.getMessage());
		} finally {
	...
	@Override
	public Stray_Ani_photosVO findByPrimaryKey(String  aPK_NO){
    ...
    public List<Stray_Ani_photosVO> getAll() {
        ...
        try {
            ...
            while (rs.next()) {
                // stray_ani_photosVO 也稱為 Domain objects
                stray_ani_photosVO = new Stray_Ani_photosVO();
                stray_ani_photosVO.setStr_Ani_Pic_No(rs.getString("str_Ani_Pic_No"));
                stray_ani_photosVO.setStray_Ani_Id(rs.getString("stray_Ani_Id"));
                stray_ani_photosVO.setMem_Id(rs.getString("mem_Id"));
                ...
                list.add(stray_ani_photosVO); // Store the row in the vector
            }
    ...
    public List<Stray_Ani_photosVO> getAll(Map<String, String[]> map) {
        ...
        try {
            ...
            con = ds.getConnection();
            String finalSQL = "select * from Stray_Ani_photos "
                  + jdbcUtil_CompositeQuery_Stray_Ani_photos.get_WhereCondition(map)
                  + "order by str_Ani_Pic_No";
            ...
            while (rs.next()) {
                ...
                list.add(stray_ani_photosVO); // Store the row in the List
            }
    ...
    public Set<Stray_Ani_photosVO> getStray_Ani_photossByStray_Ani_Id(String stray_Ani_Id) {
        Set<Stray_Ani_photosVO> set = new LinkedHashSet<Stray_Ani_photosVO>();
        Stray_Ani_photosVO stray_ani_photosVO = null;
        ...
        try {
            ...
            con = ds.getConnection();
            pstmt = con.prepareStatement(GET_Stray_Ani_photoss_ByStray_Ani_Id_STMT);
            ...
            while (rs.next()) {
                ...
                set.add(stray_ani_photosVO); // Store the row in the vector
            }
    ...
    public Set<Stray_Ani_photosVO> getStray_Ani_photossByMem_Id(String mem_Id) {
        ...
        try {
            ...
            con = ds.getConnection();
            pstmt = con.prepareStatement(GET_Stray_Ani_photoss_ByMem_Id_STMT);
