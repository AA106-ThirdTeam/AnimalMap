...
import jdbc.util.CompositeQuery.jdbcUtil_CompositeQuery_Adopt_Ani;
/** 
 *表格名稱 : <br>
 *	送養動物<br>
 *	英文:adopt_Ani<br>
...
public class Adopt_AniDAO implements Adopt_AniDAO_interface{
	...
	//====以下是一般指令====
	public static final String INSERT_STMT = "INSERT INTO adopt_Ani(adopt_Ani_Id,mem_Id,adopt_Ani_name,adopt_Ani_type,adopt_Ani_gender,adopt_Ani_heal,adopt_Ani_Vac,adopt_Ani_color,adopt_Ani_body,adopt_Ani_age,adopt_Ani_Neu,adopt_Ani_chip,adopt_Ani_date,adopt_Ani_status,adopt_Ani_CreDate,adopt_Ani_FinLat,adopt_Ani_FinLon,adopt_Ani_city,adopt_Ani_town,adopt_Ani_road,adopt_Ani_like ) VALUES  ( adopt_Ani_seq1.nextval , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?  )  " ; 
	//====以下是更新指令====
	public static final String UPDATE = "UPDATE adopt_Ani SET adopt_Ani_name=?,adopt_Ani_type=? ,adopt_Ani_gender=? ,adopt_Ani_heal=? ,adopt_Ani_Vac=? ,adopt_Ani_color=? ,adopt_Ani_body=? ,adopt_Ani_age=? ,adopt_Ani_Neu=? ,adopt_Ani_chip=? ,adopt_Ani_date=? ,adopt_Ani_status=? ,adopt_Ani_CreDate=? ,adopt_Ani_FinLat=? ,adopt_Ani_FinLon=? ,adopt_Ani_city=? ,adopt_Ani_town=? ,adopt_Ani_road=? ,adopt_Ani_like=?  WHERE adopt_Ani_Id=? " ; 
	//====以下是刪除指令====
	public static final String DELETE = "DELETE FROM adopt_Ani WHERE adopt_Ani_Id=? " ; 
	//====以下是單筆資料查詢指令====
	public static final String GET_ONE_STMT = "SELECT adopt_Ani_Id,mem_Id,adopt_Ani_name,adopt_Ani_type,adopt_Ani_gender,adopt_Ani_heal,adopt_Ani_Vac,adopt_Ani_color,adopt_Ani_body,adopt_Ani_age,adopt_Ani_Neu,adopt_Ani_chip,to_char(adopt_Ani_date,'yyyy-mm-dd') adopt_Ani_date,adopt_Ani_status,to_char(adopt_Ani_CreDate,'yyyy-mm-dd') adopt_Ani_CreDate,adopt_Ani_FinLat,adopt_Ani_FinLon,adopt_Ani_city,adopt_Ani_town,adopt_Ani_road,adopt_Ani_like FROM adopt_ani WHERE adopt_Ani_Id=? " ; 
	//====以下是單筆資料查詢指令====
	public static final String GET_ALL_STMT = "SELECT adopt_Ani_Id,mem_Id,adopt_Ani_name,adopt_Ani_type,adopt_Ani_gender,adopt_Ani_heal,adopt_Ani_Vac,adopt_Ani_color,adopt_Ani_body,adopt_Ani_age,adopt_Ani_Neu,adopt_Ani_chip,to_char(adopt_Ani_date,'yyyy-mm-dd') adopt_Ani_date,adopt_Ani_status,to_char(adopt_Ani_CreDate,'yyyy-mm-dd') adopt_Ani_CreDate,adopt_Ani_FinLat,adopt_Ani_FinLon,adopt_Ani_city,adopt_Ani_town,adopt_Ani_road,adopt_Ani_like FROM adopt_Ani order by adopt_Ani_Id " ; 

	//====以下是GET_ByFK_STMT指令====
    private static final String GET_Adopt_Anis_ByMem_Id_STMT = 
		"SELECT adopt_Ani_Id,mem_Id,adopt_Ani_name,adopt_Ani_type,adopt_Ani_gender,adopt_Ani_heal,adopt_Ani_Vac,adopt_Ani_color,adopt_Ani_body,adopt_Ani_age,adopt_Ani_Neu,adopt_Ani_chip,to_char(adopt_Ani_date,'yyyy-mm-dd') adopt_Ani_date,adopt_Ani_status,to_char(adopt_Ani_CreDate,'yyyy-mm-dd') adopt_Ani_CreDate,adopt_Ani_FinLat,adopt_Ani_FinLon,adopt_Ani_city,adopt_Ani_town,adopt_Ani_road,adopt_Ani_like FROM Adopt_Ani WHERE mem_Id = ? order by mem_Id";
	...
	private static final String UPDATE_ADOPT_ANI_LIKE =" UPDATE adopt_Ani set ADOPT_ANI_LIKE=?  WHERE adopt_Ani_Id=? " ; 
	//====以下是改寫insert方法====
	@Override
	public void insert(Adopt_AniVO aAdopt_AniVO){
		...
		try {
			...
			pstmt.setInt (20, aAdopt_AniVO.getAdopt_Ani_like());
			pstmt.executeUpdate();
	...
	public void update(Adopt_AniVO aAdopt_AniVO){
		...
		try {
			...
			pstmt.setInt (19, aAdopt_AniVO.getAdopt_Ani_like());
			pstmt.setString (20, aAdopt_AniVO.getAdopt_Ani_Id());
			pstmt.executeUpdate();
		} catch (SQLException se) {
			throw new RuntimeException("A database error occured. " + se.getMessage());
		} finally {
	...
	@Override
	public Adopt_AniVO findByPrimaryKey(String  aPK_NO){
		...
		try {
			...
			while (rs.next()) {
				...
				adopt_aniVO.setAdopt_Ani_like(rs.getInt("adopt_Ani_like"));
			}
		} catch (SQLException se) {
			throw new RuntimeException("A database error occured. " + se.getMessage());
		} finally {
    ...
    public List<Adopt_AniVO> getAll() {
        ...
        try {
            ...
            while (rs.next()) {
                // adopt_aniVO 也稱為 Domain objects
                adopt_aniVO = new Adopt_AniVO();
                adopt_aniVO.setAdopt_Ani_Id(rs.getString("adopt_Ani_Id"));
                adopt_aniVO.setMem_Id(rs.getString("mem_Id"));
                adopt_aniVO.setAdopt_Ani_name(rs.getString("adopt_Ani_name"));
                ...
                list.add(adopt_aniVO); // Store the row in the vector
            }
    ...
    public List<Adopt_AniVO> getAll(Map<String, String[]> map) {
        ...
        try {
            ...
            con = ds.getConnection();
            String finalSQL = "select * from Adopt_Ani "
                  + jdbcUtil_CompositeQuery_Adopt_Ani.get_WhereCondition(map)
                  + "order by adopt_Ani_Id";
            ...
            while (rs.next()) {
                ...
                list.add(adopt_aniVO); // Store the row in the List
            }
    ...
    public Set<Adopt_AniVO> getAdopt_AnisByMem_Id(String mem_Id) {
        Set<Adopt_AniVO> set = new LinkedHashSet<Adopt_AniVO>();
        Adopt_AniVO adopt_aniVO = null;
        ...
        try {
            ...
            con = ds.getConnection();
            pstmt = con.prepareStatement(GET_Adopt_Anis_ByMem_Id_STMT);
            ...
            while (rs.next()) {
                ...
                set.add(adopt_aniVO); // Store the row in the vector
            }
