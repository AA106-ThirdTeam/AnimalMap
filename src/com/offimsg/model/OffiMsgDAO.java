...
import jdbc.util.CompositeQuery.jdbcUtil_CompositeQuery_OffiMsg;
/** 
 *表格名稱 : <br>
 *	公告訊息<br>
 *	英文:offiMsg<br>
...
public class OffiMsgDAO implements OffiMsgDAO_interface{
	...
	//====以下是GET_ByFK_STMT指令====
    private static final String GET_OffiMsgs_ByEmp_No_STMT = 
		"SELECT offiMsg_Id,offiMsg_empId,offiMsg_Title,offiMsg_Content,to_char(offiMsg_Date,'yyyy-mm-dd') offiMsg_Date FROM OffiMsg WHERE offiMsg_empId = ? order by offiMsg_empId";
    ...
    public List<OffiMsgVO> getAll(Map<String, String[]> map) {
        ...
        try {
            ...
            con = ds.getConnection();
            String finalSQL = "select * from OffiMsg "
                  + jdbcUtil_CompositeQuery_OffiMsg.get_WhereCondition(map)
                  + "order by offiMsg_Id";
    ...
    public Set<OffiMsgVO> getOffiMsgsByEmp_No(String offiMsg_empId) {
        Set<OffiMsgVO> set = new LinkedHashSet<OffiMsgVO>();
        OffiMsgVO offimsgVO = null;
        ...
        try {
            ...
            con = ds.getConnection();
            pstmt = con.prepareStatement(GET_OffiMsgs_ByEmp_No_STMT);
            pstmt.setString(1, offiMsg_empId);
            rs = pstmt.executeQuery();
            ...
            while (rs.next()) {
                ...
                set.add(offimsgVO); // Store the row in the vector
            }
