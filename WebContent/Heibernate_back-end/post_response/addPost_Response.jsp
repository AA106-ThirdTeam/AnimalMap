...
<html>
<head>
<title>討論區留言新增 - addPost_Response.jsp</title>
...
<table border='1' cellpadding='5' cellspacing='0' width='800'>
	<tr bgcolor='#CCCCFF' align='center' valign='middle' height='20'>
		<td>
		<h3>討論區留言新增 - addPost_Response.jsp</h3>
		</td>
		<td>
		   <a href="<%=request.getContextPath()%>/Heibernate_back-end/post_response/select_page.jsp"><img src="<%=request.getContextPath()%>/images/tomcat.gif" width="100" height="100" border="1">回首頁</a>
	    </td>
	</tr>
</table>
<h3>討論區留言:</h3>
...
<table border="0">
	...
	<tr>
		...
		<td><select size="1" name="mem_Id">
			<c:forEach var="memVO" items="${memSvc.all}">
				<option value="${memVO.mem_Id}" ${(post_responseVO.memVO.mem_Id==memVO.mem_Id)? 'selected':'' } >${memVO.mem_Id}
			</c:forEach>
		</select></td>
	</tr>
	...
	<tr>
		...
		<td><select size="1" name="post_Id">
			<c:forEach var="postVO" items="${postSvc.all}">
				<option value="${postVO.post_Id}" ${(post_responseVO.postVO.post_Id==postVO.post_Id)? 'selected':'' } >${postVO.post_Id}
			</c:forEach>
		</select></td>
	</tr>
	<tr>
		<td>文章留言內容:</td>
		<td><input type="TEXT" name="post_Response_content" size="45"
	...
	<tr>
		...
		<td bgcolor="#CCCCFF">
			...
			onFocus="this.blur()" size="9" readonly type="text" name="post_time" value="<%= (post_responseVO==null)? date_post_time : post_responseVO.getPost_time()%>">
			<a class="so-BtnLink"
	...
	<tr>
		...
		<td bgcolor="#CCCCFF">
		    <input class="cal-TextBox"
			onFocus="this.blur()" size="9" readonly type="text" name="post_Response_upDate" value="<%= (post_responseVO==null)? date_post_Response_upDate : post_responseVO.getPost_Response_upDate()%>">
			<a class="so-BtnLink"
			href="javascript:calClick();return false;"
			onmouseover="calSwapImg('BTN_date', 'img_Date_OVER',true);"
			onmouseout="calSwapImg('BTN_date', 'img_Date_UP',true);"
