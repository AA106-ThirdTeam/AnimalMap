...
<jsp:useBean id="listAdopt_Anis_ByCompositeQuery" scope="request" type="java.util.List" />
...
<html>
<head>
<title>複合查詢 - listAdopt_Anis_ByCompositeQuery.jsp</title>
	<!-- 共用HEAD -->
...
<table border='1' cellpadding='5' cellspacing='0' width='800'>
    <tr bgcolor='#CCCCFF' align='center' valign='middle' height='20'>
        <td>
        <h3><font color=red>複合查詢</font>送養動物 - listAdopt_Anis_ByCompositeQuery.jsp</h3>
        <a href="<%=request.getContextPath()%>/back-end/adopt_ani/select_page.jsp"><img src="<%=request.getContextPath()%>/back-end/images/back1.gif" width="100" height="32" border="0">回首頁</a>
        </td>
    </tr>
</table>
...
<%  
    rowNumber=listAdopt_Anis_ByCompositeQuery.size();
...
<!-- page1_ByCompositeQuery end -->
    ...
    <c:forEach var="adopt_aniVO" items="${listAdopt_Anis_ByCompositeQuery}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
        <tr align='center' valign='middle' ${(adopt_aniVO.adopt_Ani_Id==param.adopt_Ani_Id) ? 'bgcolor=#CCCCFF':''}><!--將修改的那一筆加入對比色而已-->    
            ...
            <td>
                <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/back-end/adopt_ani/adopt_ani.do">
 ...
 <tr>
  <%if (rowsPerPage<rowNumber) {%>
    <%if(pageIndex>=rowsPerPage){%>
        <td><A href="<%=request.getContextPath()%>/adopt_ani/adopt_ani.do?whichPage=1&action=listAdopt_Anis_ByCompositeQuery">至第一頁</A>&nbsp;</td>
        <td><A href="<%=request.getContextPath()%>/adopt_ani/adopt_ani.do?whichPage=<%=whichPage-1%>&action=listAdopt_Anis_ByCompositeQuery">上一頁 </A>&nbsp;</td>
    <%}%>
  
    <%if(pageIndex<pageIndexArray[pageNumber-1]){%>
        <td><A href="<%=request.getContextPath()%>/adopt_ani/adopt_ani.do?whichPage=<%=whichPage+1%>&action=listAdopt_Anis_ByCompositeQuery">下一頁 </A>&nbsp;</td>
        <td><A href="<%=request.getContextPath()%>/adopt_ani/adopt_ani.do?whichPage=<%=pageNumber%>&action=listAdopt_Anis_ByCompositeQuery">至最後一頁</A>&nbsp;</td>
    <%}%>
  <%}%>  
 </tr>
</table>    
 ...
 <tr> 
   ...
   <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/adopt_ani/adopt_ani.do">   
        ...
        <%}%> 
           ...
           <input type="hidden" name="action"  value="listAdopt_Anis_ByCompositeQuery">  
        </td>
   </FORM>
 </tr>
</table>
