...
public class PetGroupDAO implements PetGroup_interface {
	private static final String GET_ALL_STMT = "from PetGroupVO order by grp_Id";
	@Override
	public void insert(PetGroupVO petgroupVO) {
		Session session = HibernateUtil.getSessionFactory().getCurrentSession();
		try {
			session.beginTransaction();
			session.saveOrUpdate(petgroupVO);
	...
	public void delete(String grp_Id) {
		...
		try {
			session.beginTransaction();
//        【此時多方(宜)可採用HQL刪除】
//			Query query = session.createQuery("delete PetGroupVO where grp_Id=?");
//			query.setParameter(0, grp_Id);
//			////System.out.println("刪除的筆數=" + query.executeUpdate());
//        【或此時多方(也)可採用去除關聯關係後，再刪除的方式】
			PetGroupVO petgroupVO = new PetGroupVO();
			petgroupVO.setGrp_Id(grp_Id);
			session.delete(petgroupVO);
//        【此時多方不可(不宜)採用cascade聯級刪除】
//        【多方petgroup2.hbm.xml如果設為 cascade="all"或 cascade="delete"將會刪除所有相關資料-包括所屬部門與同部門的其它員工將會一併被刪除】
//			PetGroupVO petgroupVO = (PetGroupVO) session.get(PetGroupVO.class, grp_Id);
//			session.delete(petgroupVO);
			session.getTransaction().commit();
		} catch (RuntimeException ex) {
			session.getTransaction().rollback();
	...
	public PetGroupVO findByPrimaryKey(String grp_Id) {
		PetGroupVO petgroupVO = null;
		Session session = HibernateUtil.getSessionFactory().getCurrentSession();
		try {
			session.beginTransaction();
			petgroupVO = (PetGroupVO) session.get(PetGroupVO.class, grp_Id);
			session.getTransaction().commit();
		} catch (RuntimeException ex) {
			session.getTransaction().rollback();
			throw ex;
    ...
    public List<PetGroupVO> getAll(Map<String, String[]> map,boolean able_like) {        
        ...
        List<PetGroupVO> list = null;
        try {
            Criteria query = session.createCriteria(PetGroupVO.class);
    ...
    public List<PetGroupVO> getAll_ver02(Map<String, String[]> map,boolean able_like) {        
        ...
        List<PetGroupVO> list = null;
        try {
        	String total_str = "from PetGroupVO where ";
            Set<String> keys = map.keySet();
            int count = 0;
            for (String key : keys) {
                String value = map.get(key)[0];
	...
	public static Criteria get_aCriteria_For_AnyDB(Criteria query, String columnName,String value,boolean able_like) {
		...
		if ("GRP_TOWN".equals(columnName)){    //用於varchar
			if(able_like){
				query.add(Restrictions.like(columnName, "%"+value+"%"));
			}else{
				query.add(Restrictions.eq(columnName, value)); 
