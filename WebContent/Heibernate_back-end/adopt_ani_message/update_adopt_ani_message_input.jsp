		...
		<td>
	       <select size="1" name="adopt_Ani_Id">
	         <c:forEach var="adopt_AniVO" items="${adopt_AniSvc.all}" > 
				<option value="${adopt_AniVO.adopt_Ani_Id}" ${(adopt_ani_messageVO.adopt_AniVO.adopt_Ani_Id==adopt_AniVO.adopt_Ani_Id)?'selected':'' } >
		...
		<td>
	       <select size="1" name="mem_Id">
	         <c:forEach var="memVO" items="${memSvc.all}" > 
				<option value="${memVO.mem_Id}" ${(adopt_ani_messageVO.memVO.mem_Id==memVO.mem_Id)?'selected':'' } >
${memVO.mem_Id}				
	         </c:forEach>   
	       </select>			
		</td>
