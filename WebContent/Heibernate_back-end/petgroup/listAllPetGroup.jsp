...
<%
    ...
    List<PetGroupVO> list = petgroupSvc.getAll();
    pageContext.setAttribute("list",list);
%>
<html>
<head>
<title>所有揪團資料 - listAllPetGroup.jsp</title>
	<!-- BS -->
...
<table class="table table-hover">
    <tr bgcolor='#CCCCFF' align='center' valign='middle' height='20'>
        <td>
        <h3>所有揪團資料 - ListAllPetGroup.jsp</h3>
...
<table border='1' bordercolor='#CCCCFF' >
    ...
    <!-- END : include page1部分 -->
    <!-- 顯示查詢資料的部分 -->
    <c:forEach var="petgroupVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
	...
	<td>
		<font color=orange>${petgroupVO.memVO.mem_Id}</font>
			...
			<td>${petgroupVO.grp_name}</td>
			<td>${petgroupVO.grp_city}</td>
			<td>${petgroupVO.GRP_TOWN}</td>
			<td>${petgroupVO.grp_road}</td>
			<td>${petgroupVO.grp_EndTime}</td>
			<td>${petgroupVO.grp_StartTime}</td>
			<td>${petgroupVO.grp_CreateTime}</td>
			<td>${petgroupVO.grp_Desc}</td>
			<td>${petgroupVO.grp_Long}</td>
			<td>${petgroupVO.grp_Lat}</td>
			<td>${petgroupVO.grp_visible}</td>
