...
import jdbc.util.CompositeQuery.jdbcUtil_CompositeQuery_Adp;
/** 
 *表格名稱 : <br>
 *	領養活動<br>
 *	英文:adp<br>
...
public class AdpDAO implements AdpDAO_interface{
	...
	//====以下是GET_ByFK_STMT指令====
    private static final String GET_Adps_ByMem_Id_STMT = 
		"SELECT adp_Id,mem_Id,adp_title,adp_adp_content,to_char(adp_start_date,'yyyy-mm-dd') adp_start_date,to_char(adp_end_date,'yyyy-mm-dd') adp_end_date,to_char(adp_upDate,'yyyy-mm-dd') adp_upDate,adp_city,adp_town,adp_road,adp_lon,adp_lat FROM Adp WHERE mem_Id = ? order by mem_Id";
    ...
    public List<AdpVO> getAll(Map<String, String[]> map) {
        ...
        try {
            ...
            con = ds.getConnection();
            String finalSQL = "select * from Adp "
                  + jdbcUtil_CompositeQuery_Adp.get_WhereCondition(map)
                  + "order by adp_Id";
            ...
            while (rs.next()) {
                ...
                list.add(adpVO); // Store the row in the List
            }
    ...
    public Set<AdpVO> getAdpsByMem_Id(String mem_Id) {
        Set<AdpVO> set = new LinkedHashSet<AdpVO>();
        AdpVO adpVO = null;
        ...
        try {
            ...
            con = ds.getConnection();
            pstmt = con.prepareStatement(GET_Adps_ByMem_Id_STMT);
            ...
            while (rs.next()) {
                ...
                set.add(adpVO); // Store the row in the vector
            }
