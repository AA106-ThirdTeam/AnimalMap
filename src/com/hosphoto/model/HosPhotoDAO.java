...
import jdbc.util.CompositeQuery.jdbcUtil_CompositeQuery_HosPhoto;
/** 
 *表格名稱 : <br>
 *	診所相片<br>
 *	英文:hosPhoto<br>
...
public class HosPhotoDAO implements HosPhotoDAO_interface{
	...
	//====以下是GET_ByFK_STMT指令====
    private static final String GET_HosPhotos_ByHos_Id_STMT = 
		"SELECT hosPhoto_Id,hosPhoto_HosId,hosPhoto_photo,isDisp_HosPhoto,hosPhoto_name,hosPhoto_extent FROM HosPhoto WHERE hosPhoto_HosId = ? order by hosPhoto_HosId";
    ...
    public List<HosPhotoVO> getAll(Map<String, String[]> map) {
        ...
        try {
            ...
            con = ds.getConnection();
            String finalSQL = "select * from HosPhoto "
                  + jdbcUtil_CompositeQuery_HosPhoto.get_WhereCondition(map)
                  + "order by hosPhoto_Id";
            ...
            while (rs.next()) {
                ...
                list.add(hosphotoVO); // Store the row in the List
            }
    ...
    public Set<HosPhotoVO> getHosPhotosByHos_Id(String hosPhoto_HosId) {
        Set<HosPhotoVO> set = new LinkedHashSet<HosPhotoVO>();
        HosPhotoVO hosphotoVO = null;
        ...
        try {
            ...
            con = ds.getConnection();
            pstmt = con.prepareStatement(GET_HosPhotos_ByHos_Id_STMT);
            ...
            while (rs.next()) {
                ...
                set.add(hosphotoVO); // Store the row in the vector
            }
