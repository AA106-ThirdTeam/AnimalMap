...
import jdbc.util.CompositeQuery.jdbcUtil_CompositeQuery_AdpPhotos;
/** 
 *表格名稱 : <br>
 *	領養活動相簿<br>
 *	英文:adpPhotos<br>
...
public class AdpPhotosDAO implements AdpPhotosDAO_interface{
	...
	//====以下是更新指令====
	public static final String UPDATE = "UPDATE adpPhotos SET adpPhotosPic=? WHERE adpPhotos_Id=? " ; 
	//====以下是刪除指令====
	...
	//====以下是GET_ByFK_STMT指令====
    private static final String GET_AdpPhotoss_ByAdp_Id_STMT = 
		"SELECT adpPhotos_Id,adp_Id,adpPhotosPic FROM AdpPhotos WHERE adp_Id = ? order by adp_Id";
	...
	public void update(AdpPhotosVO aAdpPhotosVO){
		...
		try {
			...
			pstmt.setBytes (1, aAdpPhotosVO.getAdpPhotosPic());
			pstmt.setString (2, aAdpPhotosVO.getAdpPhotos_Id());
			pstmt.executeUpdate();
		} catch (SQLException se) {
			throw new RuntimeException("A database error occured. " + se.getMessage());
		} finally {
    ...
    public List<AdpPhotosVO> getAll(Map<String, String[]> map) {
        ...
        try {
            ...
            con = ds.getConnection();
            String finalSQL = "select * from AdpPhotos "
                  + jdbcUtil_CompositeQuery_AdpPhotos.get_WhereCondition(map)
                  + "order by adpPhotos_Id";
            ...
            while (rs.next()) {
                ...
                list.add(adpphotosVO); // Store the row in the List
            }
    ...
    public Set<AdpPhotosVO> getAdpPhotossByAdp_Id(String adp_Id) {
        Set<AdpPhotosVO> set = new LinkedHashSet<AdpPhotosVO>();
        AdpPhotosVO adpphotosVO = null;
        ...
        try {
            ...
            con = ds.getConnection();
            pstmt = con.prepareStatement(GET_AdpPhotoss_ByAdp_Id_STMT);
            ...
            while (rs.next()) {
                ...
                set.add(adpphotosVO); // Store the row in the vector
            }
