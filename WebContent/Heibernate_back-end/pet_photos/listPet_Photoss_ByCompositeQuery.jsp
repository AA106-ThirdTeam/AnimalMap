...
<table border='1' cellpadding='5' cellspacing='0' width='800'>
    <tr bgcolor='#CCCCFF' align='center' valign='middle' height='20'>
        ...
        <a href="<%=request.getContextPath()%>/Heibernate_back-end/pet_photos/select_page.jsp"><img src="<%=request.getContextPath()%>/Heibernate_back-end/images/back1.gif" width="100" height="32" border="0">回首頁</a>
        </td>
    </tr>
</table>
<!-- ==== ==== -->
...
<!-- page1_ByCompositeQuery end -->
    <c:forEach var="pet_photosVO" items="${listPet_Photoss_ByCompositeQuery}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
        <tr align='center' valign='middle' ${(pet_photosVO.pet_Pic_No==param.pet_Pic_No) ? 'bgcolor=#CCCCFF':''}><!--將修改的那一筆加入對比色而已-->    
         <td>${pet_photosVO.pet_Pic_No}</td>         <td>${pet_photosVO.pet_Id}</td>         <td>${pet_photosVO.mem_Id}</td>         <td>${pet_photosVO.pet_Pic}</td>         <td>${pet_photosVO.pet_Pic_name}</td>         <td>${pet_photosVO.pet_Pic_nameEX}</td>         <td>${pet_photosVO.pet_Pic_time}</td>         <td>${pet_photosVO.pet_Pic_type}</td>  
            <td>
 ...
 <tr>
  <%if (rowsPerPage<rowNumber) {%>
    <%if(pageIndex>=rowsPerPage){%>
        <td><A href="<%=request.getContextPath()%>/pet_photos/pet_photos.do?whichPage=1&action=list_ByCompositeQuery">至第一頁</A>&nbsp;</td>
        <td><A href="<%=request.getContextPath()%>/pet_photos/pet_photos.do?whichPage=<%=whichPage-1%>&action=list_ByCompositeQuery">上一頁 </A>&nbsp;</td>
    <%}%>
    <%if(pageIndex<pageIndexArray[pageNumber-1]){%>
        <td><A href="<%=request.getContextPath()%>/pet_photos/pet_photos.do?whichPage=<%=whichPage+1%>&action=list_ByCompositeQuery">下一頁 </A>&nbsp;</td>
        <td><A href="<%=request.getContextPath()%>/pet_photos/pet_photos.do?whichPage=<%=pageNumber%>&action=list_ByCompositeQuery">至最後一頁</A>&nbsp;</td>
    <%}%>
  <%}%>  
 </tr>
</table>    
