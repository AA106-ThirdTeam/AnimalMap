...
public class PostDAO implements PostDAO_interface{
	...
	//====以下是GET_ByFK_STMT指令====
    private static final String GET_Posts_ByMem_Id_STMT = 
		"SELECT post_Id,mem_Id,post_class,post_class_Id,post_title,post_content,to_char(post_time,'yyyy-mm-dd') post_time,to_char(post_upDate,'yyyy-mm-dd') post_upDate,post_resNum FROM Post WHERE mem_Id = ? order by mem_Id";
    ...
    public List<PostVO> getAll(Map<String, String[]> map) {
        ...
        try {
            ...
            con = ds.getConnection();
            String finalSQL = "select * from Post "
                  + jdbcUtil_CompositeQuery_Post.get_WhereCondition(map)
                  + "order by post_Id";
    ...
    public Set<PostVO> getPostsByMem_Id(String mem_Id) {
        ...
        try {
            ...
            con = ds.getConnection();
            pstmt = con.prepareStatement(GET_Posts_ByMem_Id_STMT);
