 ...
 *	英文:post<br>
 */ 
@WebServlet(urlPatterns = { "/back-end/post/post.do" })
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("update".equals(action)) { // 來自update_post_input.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back-end/post/update_post_input.jsp");
					failureView.forward(req, res);
					return; //程式中斷
				}
				...
				if(requestURL.equals("/back-end/post/listPosts_ByMem_Id.jsp") 
					|| requestURL.equals("/back-end/post/listAllPost.jsp")){
					req.setAttribute("listPosts_ByMem_Id",postSvc.getPostsByMem_Id(mem_Id)); // 資料庫取出的list物件,存入request
				}
				
				
				if(requestURL.equals("/back-end/post/listPosts_ByCompositeQuery.jsp")){
        ...
        if ("insert".equals(action)) { // 來自addPost.jsp的請求  
			...
			/***************************其他可能的錯誤處理**********************************/
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/back-end/post/addPost.jsp");
				failureView.forward(req, res);
			}
		}			
		...
		if ("delete".equals(action)) { 
			...
			try {
				...
				if(requestURL.equals("/back-end/post/listPosts_ByMem_Id.jsp") 
					|| requestURL.equals("/back-end/post/listAllPost.jsp")){
					req.setAttribute("listPosts_ByMem_Id",postSvc.getPostsByMem_Id(postVO.getMem_Id())); // 資料庫取出的list物件,存入request
				}
        ...
        if ("listPosts_ByCompositeQuery".equals(action)) { // 來自select_page.jsp的複合查詢請求
            List<String> errorMsgs = new LinkedList<String>();
            // Store this set in the request scope, in case we need to
            // send the ErrorPage view.
            req.setAttribute("errorMsgs", errorMsgs);
            ...
            try {
                ...
                RequestDispatcher successView = req.getRequestDispatcher("/back-end/post/listPosts_ByCompositeQuery.jsp"); // 成功轉交listPosts_ByCompositeQuery.jsp
                successView.forward(req, res);
        ...
        if ("listPosts_ByMem_Id_A".equals(action)) {
            ...
            try {
                ...
                /*************************** 2.開始查詢資料 ****************************************/
                PostService postSvc = new PostService();
                Set<PostVO> set = postSvc.getPostsByMem_Id(mem_Id);

                /*************************** 3.查詢完成,準備轉交(Send the Success view) ************/
                req.setAttribute("listPosts_ByMem_Id", set);    // 資料庫取出的list物件,存入request

                String url = null;
                if ("listPosts_ByMem_Id_A".equals(action))
                    url = "/back-end/post/listPosts_ByMem_Id.jsp";        // 成功轉交 post/listPosts_ByMem_Id.jsp
