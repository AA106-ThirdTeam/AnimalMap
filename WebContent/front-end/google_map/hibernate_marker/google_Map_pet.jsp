...
<%
    ...
    List<PetVO> list_pet =(List)session.getAttribute("list_pet");
    //pageContext.setAttribute("list_pet",list_pet);
    int pet_map_icon_size = 48;
%>
<style>
...
%>
<div id=ex_animal_map_pet_<%=vo.getPet_Id()%> value="<%=vo.getPet_Id()%>" hidden>
        ...
        <hr>
        <button onclick="show_pet_details_page('<%=vo.getPet_Id()%>')"
...
// 					    // 標記 ID
// 					    // Custom ID
					    id: 'marker_pet_<%=vo.getPet_Id()%>'
					    ,index:'<%=vo.getPet_Id()%>'
					    ,type:'pet'
// 					    // 標記的位置
// 					    // Marker location
					    ,addr: ['<%=vo.getPet_FinLat()%>', '<%=vo.getPet_FinLon()%>']
...
// 					    // Content of infoWindow
					    ,text: 
					    	'<div id ="div_pet_<%=tem_int%>">'
					    	+'<button onclick="show_pet_details_page(this.value)"'
					    	+ 'class="btn .btn-md btn-block btn-info" >詳細資料!</button>'
					    	+ '</div>'
				    	,text_html:"ex_animal_map_pet_<%=vo.getPet_Id()%>"
// 					    // 標籤文字層，顯示於標記底下
...
// 					    // 或是 Object 定義更詳細的圖示
			            ,'icon': {
			            	 // 圖示網址
			                'url': 'https://i.imgur.com/Hy5YbHl.png'
		                	// 縮放尺寸
			                ,'scaledSize': [<%=pet_map_icon_size%>, <%=pet_map_icon_size%>]
			            }
...
// 					    },
					    ...
					    ,event       : {
                            ...
                            mouseout: {
                                func: function () {
                                    //if(infowindow_pet_<%=tem_int%>!=null){
                                        //infowindow_pet_<%=tem_int%>.close(map,this);
                                    //}
                                }
                            }
					    }
...
<script type="text/javascript">
	function show_pet_details_page(pk_value) {
		var path_parameter = 'action=getOne_For_Display&Id=' + pk_value;
		var src='<%=request.getContextPath()%>/Heibernate_front-end/marker_detail_infowindow/pet_detail_page.jsp?'+path_parameter 
