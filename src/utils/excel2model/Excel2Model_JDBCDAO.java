 ...
 *
 */
public class Excel2Model_JDBCDAO {
	...
	private static void 建立JDBCDAO(String 表格名稱, List<List> list_列) throws IOException {
		// ===============
		String 中文表格名稱 = Excel2SQL_VO.linkhashMap_excel_DB_表格中文名字.get(表格名稱);
		String 主資料夾名稱 = "com";
		...
		String str = "";
		String 副檔名 = ".java";
		String 檔案路徑 = 桌面路徑 + 主資料夾名稱 + "\\" + 小寫表格名稱 + "\\" + "model" + "\\" + 大寫表格名稱 + "JDBCDAO" + 副檔名;

		// ====file處理====
		File file = new File(檔案路徑);

		/*
		 * 此時如果直接createNewFile() 會因為parent的資料夾不存在而失敗，而這時直接使用mkdirs()
		...
		str += "package " + 主資料夾名稱 + "." + 小寫表格名稱 + "." + "model;" + "\n" + "\n" + "import java.util.*;\n"
				...
				+ "表格名稱 : <br>\n" + " *	" + 中文表格名稱 + "<br>\n" + " *	英文:" + 表格名稱 + "<br>\n" + " */" + " \n"
				+ "public class " + 大寫表格名稱 + "JDBCDAO implements " + 大寫表格名稱 + "DAO_interface{" + "\n"
		...
		// str += " public void " + "delete" + (++第幾個PK) + "("+"Integer " +
		// 英文欄位名稱+");" + "\n"
		// +" public "+大寫VO類別名稱 + " " + "findByPrimaryKey"+ (第幾個PK)
		// +"("+"Integer "+ 英文欄位名稱+");" + "\n"
		// ;
		// }
		// }
	...
	private static String create_INSERT_STMT命令(String 表格名稱) {
		...
		List<List> list_列 = linkhashMap_excel_DB.get(表格名稱);
		String 第一個PK的欄位名稱 = "";
		str += "	public static final String INSERT_STMT = \"INSERT INTO " + 表格名稱 + " (";
	...
	private static String create_UPDATE命令(String 表格名稱) {
		...
		List<List> list_列 = linkhashMap_excel_DB.get(表格名稱);
		String 第一個PK的欄位名稱 = "";
		str += "	public static final String UPDATE = \"UPDATE " + 表格名稱 + " SET ";
		// 排除PK外的第一個值
	...
	private static String create_DELETE命令(String 表格名稱) {
		...
		List<List> list_列 = linkhashMap_excel_DB.get(表格名稱);
		String 第一個PK的欄位名稱 = "";
		str += "	public static final String DELETE = \"DELETE FROM " + 表格名稱;
		// 排除PK外的第一個值
		boolean flat_排除PK_FK外的第一個欄位 = false;
		for (int k1 = 0; k1 < list_列.size(); k1++) {
			List<String> list_col = list_列.get(k1);
	...
	private static String 個別UPDATE命令產生(String 表格名稱) {
		...
		for (int k = 0; k < list_列.size(); k++) {
			...
			if (!(限制條件.indexOf("PK") == -1)) {
				...
				str += "	private static final String " + "UPDATE_" + 全部大寫欄位名稱 + " =\" UPDATE " + 表格名稱 + " set "
						+ 全部大寫欄位名稱 + "=? " + " WHERE " + 第一個PK的欄位名稱 + "=? " + "\" ; \n";
			}
		}
	...
	private static String 改寫insert方法(String 表格名稱) {
		...
		str += "	@Override\n" + "	public void " + "insert(" + 大寫VO類別名稱 + " a" + 大寫表格名稱 + "VO){" + "\n"
				...
				+ "			con = ds.getConnection();\n" + "			pstmt = con.prepareStatement(" + 大寫表格名稱
				+ "DAO.INSERT_STMT);\n";
		String 第一個PK的欄位名稱 = "";
		for (int k = 0; k < list_列.size(); k++) {
			List<String> list_col = list_列.get(k);
	...
	private static String 測試main方法(String 表格名稱) {
		String str = "	//====以下是測試main方法====\n";
		String 大寫表格名稱 = 表格名稱.toUpperCase().charAt(0) + 表格名稱.substring(1);
		String 大寫VO類別名稱 = 大寫表格名稱 + "VO";
		String 小寫表格名稱 = 表格名稱.toLowerCase();
		List<List> list_列 = linkhashMap_excel_DB.get(表格名稱);
		...
		// ====HEAD部分====
		str += "	public static void main(String[] args) { \n \n" + "		" + 大寫表格名稱 + "JDBCDAO dao = new " + 大寫表格名稱
				+ "JDBCDAO(); \n \n";

		// ====CONTEXT部分====
		str += "" + "		// 新增 \n" + "		" + 大寫表格名稱 + "VO " + 小寫表格名稱 + "VO" + 第幾個VO + " = new " + 大寫表格名稱
				+ "VO();\n";
		// ====INSERT部分====
		for (int k = 0; k < list_列.size(); k++) {
			List<String> list_col = list_列.get(k);
			...
			String 資料型態 = (String) list_col.get(3);
			String 欄位長度 = (String) list_col.get(4);
			str += "		" + 小寫表格名稱 + "VO" + 第幾個VO + ".set" + 字首大寫欄位名稱 + "(" + Common.製造假資料(資料型態, 欄位長度) + ");\n";
		}
		...
		// String 小寫英文欄位名稱 = (String) list_col.get(1);
		// String 全部大寫欄位名稱 = 小寫英文欄位名稱.toUpperCase();
		// String 字首大寫欄位名稱 = 小寫英文欄位名稱.toUpperCase().charAt(0) +
		// 小寫英文欄位名稱.substring(1);
		// String 中文欄位名稱 = (String) list_col.get(2);
		// String 資料型態 = (String) list_col.get(3);
		// String 欄位長度 = (String) list_col.get(4);
		// String 限制條件 = (String) list_col.get(5);
