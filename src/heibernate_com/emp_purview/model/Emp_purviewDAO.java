...
public class Emp_purviewDAO implements Emp_purview_interface {
	private static final String GET_ALL_STMT = "from Emp_purviewVO order by emp_purview_Id";
	@Override
	...
	@Override
	public void delete(String emp_purview_Id) {
		Session session = HibernateUtil.getSessionFactory().getCurrentSession();
		try {
			session.beginTransaction();
//        【此時多方(宜)可採用HQL刪除】
//			Query query = session.createQuery("delete Emp_purviewVO where emp_purview_Id=?");
//			query.setParameter(0, emp_purview_Id);
...
//			Emp_purviewVO emp_purviewVO = (Emp_purviewVO) session.get(Emp_purviewVO.class, emp_purview_Id);
//			session.delete(emp_purviewVO);
			session.getTransaction().commit();
		} catch (RuntimeException ex) {
			session.getTransaction().rollback();
	...
	@Override
	public Emp_purviewVO findByPrimaryKey(String emp_purview_Id) {
		Emp_purviewVO emp_purviewVO = null;
		Session session = HibernateUtil.getSessionFactory().getCurrentSession();
		try {
			session.beginTransaction();
			emp_purviewVO = (Emp_purviewVO) session.get(Emp_purviewVO.class, emp_purview_Id);
			session.getTransaction().commit();
		} catch (RuntimeException ex) {
			session.getTransaction().rollback();
			throw ex;
