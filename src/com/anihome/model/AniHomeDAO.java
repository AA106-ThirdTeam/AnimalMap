...
import jdbc.util.CompositeQuery.jdbcUtil_CompositeQuery_AniHome;
/** 
 *表格名稱 : <br>
 *	動物之家<br>
 *	英文:aniHome<br>
...
public class AniHomeDAO implements AniHomeDAO_interface{
	...
	//====以下是GET_ByFK_STMT指令====
    private static final String GET_AniHomes_ByMem_Id_STMT = 
		"SELECT aniHome_Id,mem_Id,aniHome_title,aniHome_content,to_char(aniHome_start_date,'yyyy-mm-dd') aniHome_start_date,to_char(aniHome_upDate,'yyyy-mm-dd') aniHome_upDate,aniHome_city,aniHome_town,aniHome_road,aniHome_lon,aniHome_lat FROM AniHome WHERE mem_Id = ? order by mem_Id";
    ...
    public List<AniHomeVO> getAll(Map<String, String[]> map) {
        ...
        try {
            ...
            con = ds.getConnection();
            String finalSQL = "select * from AniHome "
                  + jdbcUtil_CompositeQuery_AniHome.get_WhereCondition(map)
                  + "order by aniHome_Id";
            ...
            while (rs.next()) {
                ...
                list.add(anihomeVO); // Store the row in the List
            }
    ...
    public Set<AniHomeVO> getAniHomesByMem_Id(String mem_Id) {
        Set<AniHomeVO> set = new LinkedHashSet<AniHomeVO>();
        AniHomeVO anihomeVO = null;
        ...
        try {
            ...
            con = ds.getConnection();
            pstmt = con.prepareStatement(GET_AniHomes_ByMem_Id_STMT);
            ...
            while (rs.next()) {
                ...
                set.add(anihomeVO); // Store the row in the vector
            }
