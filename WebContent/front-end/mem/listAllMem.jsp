	...
	<table border='1' cellpadding='5' cellspacing='0'>
		<tr bgcolor='#CCCCFF' align='center' valign='middle' height='20'>
			<td>
				...
				href="<%=request.getContextPath()%>/mem/select_page.jsp"><img
...
// 						System.out.println("checkRelation= " + checkRelation);
				...
				<td>
					<FORM METHOD="post"
						...
						<button type="button" ${(memVO.mem_Id==loginMemId) ? 'hidden':''} id="openChatBtn"> 傳送訊息</button>
				...
				<td>
					...
					<FORM METHOD="post"	ACTION="<%=request.getContextPath()%>/mem/mem.do" name="showPrivMsgForm${s.index}" id="${s.index}">						
						<input type="hidden" name="privMsgRec_MemId" value="${memVO.mem_Id}">
						<input type="hidden" name="requestURL" value="<%=request.getServletPath()%>">
			...
			<c:if test="${(!empty listPrivMsg_ByRecMemId)||(!empty listPrivMsg_BySendMemId)}">
	...
	<%
		if (request.getAttribute("listRelation_ByMemId") != null) {
	%>
...
<script>
	...
	var a = document.getElementById("openChatBtn");
			...
			function openChat(sendToAccount){
				...
				$.ajax({
					...
					success : function(msg1) {
						...
						$("#50").attr("id",sendToAccount)
						
						var chatArea = document.getElementById(sendToAccount);
						
						chatArea.innerHTML=msg1;
						
						var closeBtn = document.getElementsByClassName(sendToAccount)[0];
						var messagesArea = document.getElementById("messagesArea");
					    console.log("startChatMsg on SHOW"+startChatMsg);
						...
						for (var i = 0; i < jsonData.recievedJsonArray.length; i++) {
						    ...
						    var showedData = userName + ": " + message + "     "+sendTime+"\r\n";
					        
					        messagesArea.value = messagesArea.value + showedData;
						    
						    console.log(userName+message+sendTime);
						...
						document.getElementById("sendMessage").addEventListener("click",function(){
							    ...
							    var URLs = "<%=request.getContextPath()%>/privMsg/privMsg.do";
							    var sendData ="action=insert" +"&privMsgRec_MemId=" + userName + "&privMsgRec_MemId=" + "1000000" +"&requestURL=<%=request.getServletPath()%>";
						...
						webSocket.onmessage = function(event) {
					        ...
					        var message = jsonObj.userName + ": " + jsonObj.message + "     "+displayTime+"\r\n";
