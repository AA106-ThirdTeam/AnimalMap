...
<!-- 						alt="Nature" style="width: 100%"> -->
				...
				<form METHOD="post" ACTION="<%=request.getContextPath()%>/front-end/post/post.do" name="Posts">
						<h3>
<%-- 						發文者 :<%=postVO.getMem_Id() %> <a href="#" style="color: rgba(255, 0, 0, 0.49);"></a> --%>
						...
						<h4><td>文章標題：<input type="TEXT" name="post_title" size="35" 
							...
							value="<%=(postVO == null) ? "請輸入標題" : postVO.getPost_title() %>" /></td>
						</h4>
					...
					<div style="text-align: right" >
						...
						<input type="submit" value="送出新增" class="w3-btn w3-padding-large w3-white w3-border w3-hover-border-black">
					</div>
					<img src="<%=request.getContextPath()%>/front-end/post/images/house.png" onClick="AddPostMagical()">
...
<script>
function AddPostMagical() {
		Posts.post_title.value="領養狗狗的心得";
		Posts.post_content.value="經過這一次的出遊，領養了一隻臘腸狗很可愛，給他一個可愛的名字叫＂小白　＂！<br>這隻可愛的臘腸狗＂小白寶貝＂是在路上撿到的<br>因為她實在很乖 不太叫也很親人 所以怕是走失的<br>不過等一陣子後還是沒有飼主聯絡 所以就帶她去打晶片了<br>剛開始養他的時候 瘦得不成狗樣 看起來怪可憐的QQ<br>連打噴嚏都會跌倒 只有3.9公斤!!!<br>其實一開始有猶豫 到底要一輩子養他還是當中途<br>雖然老家有領養另外一隻狗 有照顧經驗<br>但是畢竟我還是學生 如果決定要養他的話算是負擔<br>也怕吵到鄰居(房東允許養)<br>但挺過一開始的過渡期,後來都是滿滿的幸福感~~~~<br>尤其看他從那麼虛弱變成 頭好壯壯的樣子 真是很有成就感<br>希望狗狗們都能遇到愛他的主人 過得很幸福!!<br>謝謝收看";
	}
