...
//			session.delete(second_prodphotosVO);
    ...
    public List<Second_ProdPhotosVO> getAll_ver02(Map<String, String[]> map,boolean able_like) {        
        Session session = HibernateUtil.getSessionFactory().getCurrentSession();
        Transaction tx = session.beginTransaction();
        List<Second_ProdPhotosVO> list = null;
        try {
        	String total_str = "from Second_ProdPhotosVO where ";
            Set<String> keys = map.keySet();
            int count = 0;
            for (String key : keys) {
                String value = map.get(key)[0];
	...
	public static Criteria get_aCriteria_For_AnyDB(Criteria query, String columnName,String value,boolean able_like) {
		...
		if ("second_ProdPhotos_Pic".equals(columnName))    //用於byte[]
