 ...
 *	英文:pet_Message<br>
 */ 
@WebServlet(urlPatterns = { "/back-end/pet_message/pet_message.do" })
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("getOne_For_Display".equals(action)) { 
			...
			try {
				...
				String url = "/back-end/pet_message/listOnePet_Message.jsp";
		...
		if ("getOne_For_Update".equals(action)) {
			...
			try {			
				...
				/***************************3.查詢完成,準備轉交(Send the Success view)*************/
				req.setAttribute("pet_messageVO", pet_messageVO); 		
				String url = "/back-end/pet_message/update_pet_message_input.jsp";
		...
		if ("update".equals(action)) { // 來自update_pet_Message_input.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back-end/pet_message/update_pet_message_input.jsp");
					failureView.forward(req, res);
					return; //程式中斷
				}
				...
				if(requestURL.equals("/back-end/pet_message/listPet_Messages_ByPet_Id.jsp") 
					|| requestURL.equals("/back-end/pet_message/listAllPet_Message.jsp")){
					req.setAttribute("listPet_Messages_ByPet_Id",pet_messageSvc.getPet_MessagesByPet_Id(pet_Id)); // 資料庫取出的list物件,存入request
				}

				if(requestURL.equals("/back-end/pet_message/listPet_Messages_ByMem_Id.jsp") 
					|| requestURL.equals("/back-end/pet_message/listAllPet_Message.jsp")){
					req.setAttribute("listPet_Messages_ByMem_Id",pet_messageSvc.getPet_MessagesByMem_Id(mem_Id)); // 資料庫取出的list物件,存入request
				}
				
				
				if(requestURL.equals("/back-end/pet_message/listPet_Messages_ByCompositeQuery.jsp")){
        ...
        if ("insert".equals(action)) { // 來自addPet_Message.jsp的請求  
			...
			try {
                ...
                if (!errorMsgs.isEmpty()) {
                    req.setAttribute("pet_messageVO", pet_messageVO); // 含有輸入格式錯誤的pet_messageVO物件,也存入req
                    RequestDispatcher failureView = req.getRequestDispatcher("/back-end/pet_message/addPet_Message.jsp");
                    failureView.forward(req, res);
                    return;
                }
				...
				String url = "/back-end/pet_message/listAllPet_Message.jsp";
			...
			/***************************其他可能的錯誤處理**********************************/
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/back-end/pet_message/addPet_Message.jsp");
				failureView.forward(req, res);
			}
		}			
		...
		if ("delete".equals(action)) { 
			...
			try {
				...
				if(requestURL.equals("/back-end/pet_message/listPet_Messages_ByPet_Id.jsp") 
					|| requestURL.equals("/back-end/pet_message/listAllPet_Message.jsp")){
					req.setAttribute("listPet_Messages_ByPet_Id",pet_messageSvc.getPet_MessagesByPet_Id(pet_messageVO.getPet_Id())); // 資料庫取出的list物件,存入request
				}

				if(requestURL.equals("/back-end/pet_message/listPet_Messages_ByMem_Id.jsp") 
					|| requestURL.equals("/back-end/pet_message/listAllPet_Message.jsp")){
					req.setAttribute("listPet_Messages_ByMem_Id",pet_messageSvc.getPet_MessagesByMem_Id(pet_messageVO.getMem_Id())); // 資料庫取出的list物件,存入request
				}
        ...
        if ("listPet_Messages_ByCompositeQuery".equals(action)) { // 來自select_page.jsp的複合查詢請求
            List<String> errorMsgs = new LinkedList<String>();
            // Store this set in the request scope, in case we need to
            // send the ErrorPage view.
            req.setAttribute("errorMsgs", errorMsgs);
            ...
            try {
                ...
                RequestDispatcher successView = req.getRequestDispatcher("/back-end/pet_message/listPet_Messages_ByCompositeQuery.jsp"); // 成功轉交listPet_Messages_ByCompositeQuery.jsp
                successView.forward(req, res);
        ...
        // 來自select_page.jsp的請求                                  // 來自 pet_message/listAllPet_Message.jsp的請求
        if ("listPet_Messages_ByPet_Id_A".equals(action)) {
            ...
            try {
                ...
                /*************************** 2.開始查詢資料 ****************************************/
                Pet_MessageService pet_messageSvc = new Pet_MessageService();
                Set<Pet_MessageVO> set = pet_messageSvc.getPet_MessagesByPet_Id(pet_Id);

                /*************************** 3.查詢完成,準備轉交(Send the Success view) ************/
                req.setAttribute("listPet_Messages_ByPet_Id", set);    // 資料庫取出的list物件,存入request

                String url = null;
                if ("listPet_Messages_ByPet_Id_A".equals(action))
                    url = "/back-end/pet_message/listPet_Messages_ByPet_Id.jsp";        // 成功轉交 pet_message/listPet_Messages_ByPet_Id.jsp
        ...
        // 來自select_page.jsp的請求                                  // 來自 pet_message/listAllPet_Message.jsp的請求
        if ("listPet_Messages_ByMem_Id_A".equals(action)) {
            ...
            try {
                ...
                /*************************** 2.開始查詢資料 ****************************************/
                Pet_MessageService pet_messageSvc = new Pet_MessageService();
                Set<Pet_MessageVO> set = pet_messageSvc.getPet_MessagesByMem_Id(mem_Id);

                /*************************** 3.查詢完成,準備轉交(Send the Success view) ************/
                req.setAttribute("listPet_Messages_ByMem_Id", set);    // 資料庫取出的list物件,存入request

                String url = null;
                if ("listPet_Messages_ByMem_Id_A".equals(action))
                    url = "/back-end/pet_message/listPet_Messages_ByMem_Id.jsp";        // 成功轉交 pet_message/listPet_Messages_ByMem_Id.jsp
