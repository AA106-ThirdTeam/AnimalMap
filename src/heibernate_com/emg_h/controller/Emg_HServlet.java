package heibernate_com.emg_h.controller;
import java.io.*;
import java.util.*;
...
@WebServlet(urlPatterns = { "/Heibernate_back-end/emg_h/emg_h.do" })
public class Emg_HServlet extends HttpServlet {
		...
		try {
			...
			if (!errorMsgs.isEmpty()) {
				RequestDispatcher failureView = req
						.getRequestDispatcher("/Heibernate_back-end/emg_h/select_page.jsp");
			...
			req.setAttribute("emg_hVO", emg_hVO); // 資料庫取出的emg_hVO物件,存入req
			String url = "/Heibernate_back-end/emg_h/listOneEmg_H.jsp";
	...
	private void getOne_For_Update(HttpServletRequest req, HttpServletResponse res)throws ServletException, IOException {
		...
		try {
			...
			/***************************3.查詢完成,準備轉交(Send the Success view)************/
			req.setAttribute("emg_hVO", emg_hVO); // 資料庫取出的emg_hVO物件,存入req
			String url = "/Heibernate_back-end/emg_h/update_emg_h_input.jsp";
	...
	private void update(HttpServletRequest req, HttpServletResponse res)throws ServletException, IOException {
		...
		try {
			...
			if (!errorMsgs.isEmpty()) {
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/Heibernate_back-end/emg_h/update_emg_h_input.jsp");
				failureView.forward(req, res);
				return; //程式中斷
			}
			/***************************2.開始修改資料*****************************************/
			...
			//if(requestURL.equals("/Heibernate_back-end/emg_h/listEmg_Hs_ByMem_Id.jsp") 
				//|| requestURL.equals("/Heibernate_back-end/emg_h/listAllEmg_H.jsp")){
				//req.setAttribute("listEmg_Hs_ByMem_Id",emg_hSvc.getEmg_HsByMem_Id(mem_Id)); // 資料庫取出的list物件,存入request
			//}
			//if(requestURL.equals("/Heibernate_back-end/emg_h/listEmg_Hs_ByCompositeQuery.jsp")){
	...
	private void insert(HttpServletRequest req, HttpServletResponse res)throws ServletException, IOException {
		...
		try {
			/***********************1.接收請求參數 - 輸入格式的錯誤處理*************************/
               ...
               if (!errorMsgs.isEmpty()) {
                   req.setAttribute("emg_hVO", emg_hVO); // 含有輸入格式錯誤的emg_hVO物件,也存入req
                   RequestDispatcher failureView = req.getRequestDispatcher("/Heibernate_back-end/emg_h/addEmg_H.jsp");
                   failureView.forward(req, res);
                   return;
               }
               /***************************2.開始新增資料***************************************/
			...
			String url = "/Heibernate_back-end/emg_h/listAllEmg_H.jsp";
		...
		/***************************其他可能的錯誤處理**********************************/
			...
			RequestDispatcher failureView = req
					.getRequestDispatcher("/Heibernate_back-end/emg_h/addEmg_H.jsp");
			failureView.forward(req, res);
		}
	}
	private void delete(HttpServletRequest req, HttpServletResponse res)throws ServletException, IOException {
		...
		try {
			...
			if(requestURL.equals("/mem/listEmg_Hs_ByMem_Id.jsp") || requestURL.equals("/mem/listAllMem.jsp")){
			  //req.setAttribute("listEmg_Hs_ByMem_Id",memSvc.getEmg_HsByMem_Id(emg_hVO.getMem_Id())); // 資料庫取出的list物件,存入request
			  //req.setAttribute("listEmg_Hs_ByMem_Id",memSvc.getEmg_HsByMem_Id(emg_hVO.getMemVO().getMem_Id())); // 資料庫取出的list物件,存入request
			}
			String url = requestURL;
