...
<table border='1' cellpadding='5' cellspacing='0' width='800'>
    <tr bgcolor='#CCCCFF' align='center' valign='middle' height='20'>
        ...
        <a href="<%=request.getContextPath()%>/Heibernate_back-end/emp/select_page.jsp"><img src="<%=request.getContextPath()%>/Heibernate_back-end/images/back1.gif" width="100" height="32" border="0">回首頁</a>
        </td>
    </tr>
</table>
<!-- ==== ==== -->
<table border='1' bordercolor='#CCCCFF'>
    <tr>
     <th>員工編號</th>     <th>員工姓名</th>     <th>員工密碼</th>     <th>員工信箱</th>     <th>員工身分證</th>     <th>員工出生年月日</th>     <th>員工電話</th>     <th>員工地址</th>     <th>員工狀態</th>     <th>員工照片</th>     <th>員工照片副檔名</th>     <th>雇用日期</th>     <th>離職日期</th>  
        <th>修改</th>
        <th>刪除</th>
    </tr>
    ...
    <c:forEach var="empVO" items="${listEmps_ByCompositeQuery}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
        <tr align='center' valign='middle' ${(empVO.emp_No==param.emp_No) ? 'bgcolor=#CCCCFF':''}><!--將修改的那一筆加入對比色而已-->    
         <td>${empVO.emp_No}</td>         <td>${empVO.emp_name}</td>         <td>${empVO.emp_Pw}</td>         <td>${empVO.emp_email}</td>         <td>${empVO.emp_Id}</td>         <td>${empVO.emp_birthday}</td>         <td>${empVO.emp_phone}</td>         <td>${empVO.emp_address}</td>         <td>${empVO.emp_status}</td>         <td>${empVO.emp_picture}</td>         <td>${empVO.emp_Pic_format}</td>         <td>${empVO.emp_hiredate}</td>         <td>${empVO.emp_firedate}</td>  
            <td>
                <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/Heibernate_back-end/emp/emp.do">
 ...
 <tr>
  <%if (rowsPerPage<rowNumber) {%>
    <%if(pageIndex>=rowsPerPage){%>
        <td><A href="<%=request.getContextPath()%>/emp/emp.do?whichPage=1&action=list_ByCompositeQuery">至第一頁</A>&nbsp;</td>
        <td><A href="<%=request.getContextPath()%>/emp/emp.do?whichPage=<%=whichPage-1%>&action=list_ByCompositeQuery">上一頁 </A>&nbsp;</td>
    <%}%>
    <%if(pageIndex<pageIndexArray[pageNumber-1]){%>
        <td><A href="<%=request.getContextPath()%>/emp/emp.do?whichPage=<%=whichPage+1%>&action=list_ByCompositeQuery">下一頁 </A>&nbsp;</td>
        <td><A href="<%=request.getContextPath()%>/emp/emp.do?whichPage=<%=pageNumber%>&action=list_ByCompositeQuery">至最後一頁</A>&nbsp;</td>
    <%}%>
  <%}%>  
 </tr>
</table>    
