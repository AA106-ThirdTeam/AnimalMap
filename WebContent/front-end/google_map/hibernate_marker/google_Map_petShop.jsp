...
<%
    ...
    List<PetShopVO> list_petshop =(List)session.getAttribute("list_petShop");
    //pageContext.setAttribute("list_petshop",list_petshop);
    int petshop_map_icon_size = 48;
%>
<style>
...
%>
<div id=ex_animal_map_petShop_<%=vo.getShop_Id()%> value="<%=vo.getShop_Id()%>" hidden>
        <div style="width: 20vw;">
            <div class="">
                ...
                <hr>
                <p>
        			<%=vo.getShop_StartTime()%>
        		</p>
            </div>
        </div> 		
        <hr>
        <button onclick="show_petShop_details_page('<%=vo.getShop_Id()%>')"
...
// 					    // 標記 ID
// 					    // Custom ID
					    id: 'marker_petshop_<%=vo.getShop_Id()%>'
					    ,index:'<%=vo.getShop_Id()%>'
					    ,type:'petshop'
// 					    // 標記的位置
// 					    // Marker location
					    ,addr: ['<%=vo.getShop_Long()%>', '<%=vo.getShop_Lat()%>']
...
// 					    // Content of infoWindow
					    ,text: 
					    	'<div id ="div_petShop_<%=tem_int%>">'
					    	+'<button onclick="show_petShop_details_page(this.value)"'
					    	+ 'class="btn .btn-md btn-block btn-info" >詳細資料!</button>'
					    	+ '</div>'
				    	,text_html:"ex_animal_map_petShop_<%=vo.getShop_Id()%>"
// 					    // 標籤文字層，顯示於標記底下
...
// 					    // 或是 Object 定義更詳細的圖示
			            ,'icon': {
			            	 // 圖示網址
			                'url': 'https://i.imgur.com/1Cm0kdo.png'
		                	// 縮放尺寸
			                ,'scaledSize': [<%=petshop_map_icon_size%>, <%=petshop_map_icon_size%>]
			            }
...
// 					    },
					    ...
					    ,event       : {
                            ...
                            mouseout: {
                                func: function () {
                                    //if(infowindow_petShop_<%=tem_int%>!=null){
                                        //infowindow_petShop_<%=tem_int%>.close(map,this);
                                    //}
                                }
                            }
					    }
...
<script type="text/javascript">
	function show_petShop_details_page(pk_value) {
		var path_parameter = 'action=getOne_For_Display&Id=' + pk_value;
		var src='<%=request.getContextPath()%>/Heibernate_front-end/marker_detail_infowindow/petShop_detail_page.jsp?'+path_parameter 
